...
def build_email_text(oslovenie, meno, typ, poistovna, zmluva, vyrocie_pz):
    ...
    return (
        ...
        f"Výročie tejto poistnej zmluvy je {vyrocie_pz}. Pravdepodobne Vám do mailu prišiel nový predpis na platbu tejto zmluvy.\n"
        f"Neprehliadnite dátum zaplatenia poistnej zmluvy. V prípade nezaplatenia, zmluva zaniká. Spoločne by sme tak museli riešiť proces uzatvárania zmluvy a vinkulácie nanovo.\n\n"
...
@app.route("/")
def home():
    return render_template_string(APP_TEMPLATE, content="<p>Víta Vás ProHypo Asistent. Vyber si modul v ktorom chceš pracovať."
    "</p>")
...
def vystupny_mail():
        ...
        if typ_mailu == "nehnutelnost":
            ...
            if not all([oslovenie, priezvisko, typ, adresa]):
                ...
                result = (
                    ...
                    f"Platobné údaje nájdete v tele mailu, ktorý Vám prišiel zo systému najpoistenie spolu so zmluvnou dokumentáciou.\n"
    ...
    content += f"<option value='rodinného dom' {'selected' if form_data['typ_nehnutelnosti']=='Rodinný dom' else ''}>Rodinný dom</option>"
